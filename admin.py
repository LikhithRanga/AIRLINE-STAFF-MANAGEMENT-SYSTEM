...
if selected_operation.startswith("Create"):
    ...
    assigned_tasks_count = count_assigned_tasks(employee_id) #fucntion call
    st.write(f"Number of Assignments: {assigned_tasks_count}")

query_status_distribution = "SELECT status, COUNT(*) FROM task GROUP BY status"
status_distribution_data = execute_query(query_status_distribution, fetchone=False)
status_distribution_df = pd.DataFrame(status_distribution_data, columns=["Status", "Count"])

if not status_distribution_df.empty:
    fig = px.pie(status_distribution_df, values='Count', names='Status', title='Task Status Distribution')
    st.plotly_chart(fig)
else:
    st.warning("No task status data available.")